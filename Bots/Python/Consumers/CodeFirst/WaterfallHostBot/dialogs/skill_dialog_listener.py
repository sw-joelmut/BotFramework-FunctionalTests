...
from Consumers.CodeFirst.SimpleHostBot.app import AppInsightsClient

class SkillDialogListener(Dialog):
    SKILLCONVERSATIONIDSTATEKEY = (
        "Microsoft.Bot.Builder.Dialogs.SkillDialog.SkillConversationId"
    )

    def __init__(self, dialog_options: SkillDialogOptions, dialog_id: str, telemetry_client: AppInsightsClient):
    ...
    async def _send_to_skill(
        ...
        self._telemetry_client.track_event("BeforePostActivityAsync", 
        {'custom_dimensions': { 
            'dialogOptions': json.dumps(self.dialog_options),
            'skillInfo': json.dumps(skill_info),
            'skillConversationId': skill_conversation_id,
        ...
        self._telemetry_client.track_event("AfterPostActivityAsync", 
        {'custom_dimensions': { 
